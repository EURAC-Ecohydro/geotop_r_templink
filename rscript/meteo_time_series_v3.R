...
ddnt3 <- dygraph(meteodf[,c(nWindVelocity,"WindAngle","WindNorth")],main=main,ylab=nWindVelocity) %>% 
  dySeries(nWindVelocity,label="WindSp",axis = 'y',color="blue") %>%  
  dySeries("WindAngle",label="WindDir", axis = 'y2',color="green")  %>% 
 # dySeries("WindNorth",axis="y2",color="#fdae61",label="North/South",fillGraph = TRUE) %>% #pointShape="ex",strokePattern="dotted",drawPoints=TRUE,stemPlot=TRUE) %>%
  dyShadow("WindNorth",axis="y2",label="North/South") %>% ### ,color="#fdae61",fillGraph = TRUE
